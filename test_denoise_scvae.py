...
from utils import GrayDataset, UltrasoundDataset, MiniImagenet, hoyer_metric, compute_indicators, slice_image, recon_image, get_noise
...
model = SCVAE(in_channels=model_args.in_channels,
...
with open(csv_filename, 'w', newline='') as csvfile:
    fieldnames = ['Name', 'Sparsity', 'PSNR1', 'SSIM1', 'NMI1', 'LPIPS1', 'PSNR2', 'SSIM2', 'NMI2', 'LPIPS2']
...
with torch.no_grad():
    for index, (image_origin, name) in enumerate(test_loader):
        ...
        image_noise = get_noise(image_origin, data_args.sigma)

        if data_args.dataset == 'gray' or data_args.dataset == 'imagenet':
            ori_shape = image_origin.shape
            patches = slice_image(image_noise, data_args.patch_size, data_args.stride_size).to(device)

            bs, _, _, _ = patches.shape
            if bs > 16:
                continue

            patches_recon, z, _, _ = model(patches)
            image_recon = recon_image(patches_recon, ori_shape, data_args.patch_size, data_args.stride_size).to(device)
        else:
            image_recon, z, _, _ = model(image_noise)

        sparsity = hoyer_metric(z).item()
        PSNR1, SSIM1, NMI1, LPIPS1 = compute_indicators(image_origin, image_noise)
        PSNR2, SSIM2, NMI2, LPIPS2 = compute_indicators(image_origin, image_recon)

        # if (index + 1) % 200 == 0:
        plot_images(image_noise,
                    image_recon,
                    image_fold_path,
                    name,
