...
from utils import GrayDataset, UltrasoundDataset, MiniImagenet, get_recon_loss, hoyer_metric, get_noise
...
for epoch in range(train_args.epochs):
    ...
    for image_origin, _ in train_loader:
        ...
        # forward
        image_recon, z, latent_loss, _ = model(image_noise)
    ...
    with torch.no_grad():
        for image_origin, _ in val_loader:
            ...
            # forward
            image_recon, z, latent_loss, dictionary = model(image_noise)
            recon_loss = get_recon_loss(image_origin, image_recon)
            sparsity = hoyer_metric(z)
