...
from utils import GrayDataset, UltrasoundDataset, MiniImagenet, compute_indicators, slice_image, recon_image
...
with open(csv_filename, 'w', newline='') as csvfile:
    fieldnames = ['Name', 'PSNR', 'SSIM', 'NMI', 'LPIPS']
...
with torch.no_grad():
    for index, (image_origin, name) in enumerate(test_loader):
        ...
        if data_args.dataset == 'gray' or data_args.dataset == 'imagenet':
            ...
            patches_recon, _, dictionary = model(patches)
            image_recon = recon_image(patches_recon, ori_shape, data_args.patch_size, data_args.stride_size).to(device)
        else:
            image_recon, _, dictionary = model(image_origin)

            plot_dict_tsne(dictionary, './', 'vqvae_tsne.png')
        break
